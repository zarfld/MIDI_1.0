...
const char* get_gm1_percussion_name(uint8_t key) {
    // GM1 percussion range: 35-81 (47 sounds)
    // Use offset array to avoid sparse initialization
    static const char* gm1_perc_names[47] = {
    ...
    // Validate range and convert to 0-based index
    if (key >= 35 && key <= 81) {
        return gm1_perc_names[key - 35];  // Offset by 35 for 0-based array
    }
    return "Unknown";
}
